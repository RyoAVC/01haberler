...
export async function generateMetadata({ params }: Props) {
 ...
 return item && item.kind === "yerel" ? { title: item.title, description: item.summary.slice(0, 160), alternates: { canonical: "/yerel/" + slug } } : { robots: { index: false } };
}
export default async function Page({ params, searchParams }: Props) {
 const [{ slug }, q] = await Promise.all([params, searchParams]);
 return <CollectionDetail kind="yerel" slug={slug} page={Math.max(1, Math.floor(Number(q.sayfa) || 1))} />;
}
