...
export default async function OperationsPage() {
  ...
  const [health, overdue] = await Promise.all([prisma.siteSetting.findUnique({ where: { key: "newsroom.health.cron" } }), prisma.article.count({ where: { status: "SCHEDULED", scheduledAt: { lt: new Date(Date.now() - 15 * 60000) } } })]);
  const cron = health?.value as { startedAt?: string; finishedAt?: string; status?: string } | undefined;
  ...
  return <div><h1 className="font-serif text-headline-l">Operasyon Merkezi</h1><p className="mt-2 text-caption text-ink-dark-secondary">Kaynak sağlığı ve son 30 haber alma işi. Bu ekran görev çalıştırmaz; yenilendiğinde güncel kayıtları okur.</p>
    ...
    <section className="module-card my-6"><h2 className="font-serif text-headline-m">Yayın ve servis kontrolü</h2><ul className="mt-3 space-y-2 text-caption"><li>Cron son durum: {cron?.status ?? "Henüz sağlık kaydı yok"}</li><li>Son başlangıç: {cron?.startedAt ? formatDateTr(new Date(cron.startedAt)) : "—"} · Son bitiş: {cron?.finishedAt ? formatDateTr(new Date(cron.finishedAt)) : "—"}</li><li>15 dakikadan fazla geciken zamanlı yayın: {overdue}</li><li>Bülten sağlayıcı ayarı: {newsletterConfigured() ? "Var" : "Eksik"} · AI sağlayıcı ayarı: {isAiEditorEnabled() ? "Var" : "Eksik"}</li></ul><p className="mt-3 text-caption">Yapılandırma durumu bağlantı testi değildir. RUNNING uzun süre değişmiyorsa cron ve sunucu loglarını inceleyin.</p></section>
